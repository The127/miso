...
func TestAnOfflineStepShowsItHasNoNetwork(t *testing.T) {
	// arrange
	planned := plan.Plan{Stages: []plan.Stage{{
		Base:  "scratch",
		Steps: []plan.Step{{Instruction: imagefile.Run{Line: 2, Offline: true, Command: "make test"}, Key: "9a8b7c6d5e4f"}},
	}}}
	var out bytes.Buffer
	...
	// assert
	require.NoError(t, err)
	assert.Equal(t, "FROM scratch\n   2  9a8b7c6d5e4f  RUN --network=none make test\n", out.String())
}
