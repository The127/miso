...
// of the key.
func (a *Agent) Run(ctx context.Context, run protocol.Run, _ io.Writer) (int, error) {
	work, err := a.layers.Begin(run.Key)
	if err != nil {
		return 0, err
	}

	code, err := a.runOn(ctx, work.Dir(), run)
	if err != nil || code != 0 {
		_ = work.Discard()
...
func (a *Agent) runOn(ctx context.Context, upper string, run protocol.Run) (int, error) {
